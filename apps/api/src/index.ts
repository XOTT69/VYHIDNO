...
async function listProducts(request: Request, env: Env, url: URL) {
  ...
  const result = await env.DB.prepare(`
    SELECT p.id,p.slug,p.name,p.brand,p.model,p.variant,p.category,p.image_url,p.reference_url,p.reference_store,
    ...
    FROM products p
    LEFT JOIN offers o ON o.id=(SELECT o2.id FROM offers o2 WHERE o2.product_id=p.id AND o2.availability IN ('in_stock','preorder') AND o2.checked_at>=datetime('now','-48 hours') ORDER BY o2.price+o2.delivery_price ASC LIMIT 1)
    LEFT JOIN stores s ON s.id=o.store_id
    LEFT JOIN deal_scores ds ON ds.offer_id=o.id
    WHERE (o.id IS NOT NULL OR p.reference_url IS NOT NULL) AND (?1='' OR p.name LIKE ?2 OR p.brand LIKE ?2 OR p.model LIKE ?2 OR p.gtin=?1 OR p.mpn=?1)
...
async function getProduct(request: Request, env: Env, slugOrId: string) {
  ...
  const offers = await env.DB.prepare(`
    ...
    WHERE o.product_id=?1 AND o.checked_at>=datetime('now','-48 hours') ORDER BY CASE WHEN o.availability='in_stock' THEN 0 ELSE 1 END,o.price+o.delivery_price ASC
...
async function listDeals(request: Request, env: Env, url: URL) {
  ...
  const result = await env.DB.prepare(`
    ...
    WHERE o.availability='in_stock' AND o.checked_at>=datetime('now','-48 hours') AND ds.real_discount_pct>0 AND ds.score>=?1 AND (?2='' OR p.category=?2)
...
async function listMapDeals(request: Request, env: Env, url: URL) {
  ...
  const result = await env.DB.prepare(`
    ...
    WHERE o.availability='in_stock' AND o.checked_at>=datetime('now','-48 hours') AND sl.lat IS NOT NULL AND sl.lng IS NOT NULL
...
async function listWatchlist(request: Request, env: Env, url: URL) {
  ...
  const result = await env.DB.prepare(`
    ...
    LEFT JOIN offers o ON o.id=(SELECT o2.id FROM offers o2 WHERE o2.product_id=p.id AND o2.availability='in_stock' AND o2.checked_at>=datetime('now','-48 hours') ORDER BY o2.price+o2.delivery_price LIMIT 1)
...
async function ingestOffer(request: Request, env: Env) {
  ...
  if (!isProductUrl(externalUrl)) return error(request, env, 'invalid_url', 400, 'externalUrl має вести на конкретний товар через HTTPS, не на головну сторінку магазину.')
